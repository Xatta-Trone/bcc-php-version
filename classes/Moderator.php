...
class Moderator{
	...
	public function addModerator($data,$file){
		//var_dump($data);
		//var_dump($file);
		//exit();
		...
		if (empty($membername)) {
			...
			if ($imgError=='0') {
				...
				$query = "INSERT INTO tbl_moderator(name,email,fb,linkedin,batch_dept,image)
	...
	public function getAllMembers(){
		$query = "SELECT * FROM tbl_moderator ORDER BY id ASC";

		return $this->db->select($query);
	}

	public function getMemberById($id){
		$query = "SELECT * FROM tbl_moderator WHERE id='$id'";
	...
	public function deleteMemberById($id){
		...
		$query = "DELETE FROM tbl_moderator WHERE id='$id'";
			...
			if (!empty($imgName)) {
				if ($imgSize>2097152) {
					...
					if ($imgError=='0') {
						...
						/*$query = "INSERT INTO tbl_moderator(name,dept,area,position,email,password,fb,linkedin,image)
						VALUES('$membername','$dept','$area','$position','$memberemail','password','$memberfb','$memberlinkedin','$uniqueName')";*/

						$query = "UPDATE tbl_moderator SET
							name = '$membername',
							email = '$memberemail',
							fb = '$memberfb',
